        ...
        if self.vRamStart > -1:
            ...
            if GlobalConfig.ADD_NEW_SYMBOLS:
                if self.context.getSymbol(startVram, False) is None and startVram not in self.context.newPointersInData:
                    contextSym = self.context.addSymbol(startVram, f"R_{startVram:08X}")
            ...
            for w in self.words:
                ...
                if partOfJumpTable:
                    ...
                    if GlobalConfig.ADD_NEW_SYMBOLS and self.context.getAnySymbol(currentVram) is None:
                        contextSym = self.context.addSymbol(currentVram, f"R_{currentVram:08X}")
                        contextSym.isAutogenerated = True
                        contextSym.isDefined = True
                        if self.bytes[offset] != 0 and contextSym.type is None:
