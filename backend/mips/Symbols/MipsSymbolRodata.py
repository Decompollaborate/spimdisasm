...
class SymbolRodata(SymbolBase):
    ...
    def isString(self, type: str) -> bool:
        if type == "char":
            ...
            if GlobalConfig.STRING_GUESSER and self.vram is not None:
                contextSym = self.context.getSymbol(self.vram, True, False)
    ...
    def analyze(self):
        if self.vram is not None:
            contextSym = self.context.getSymbol(self.vram, True, False)
            if contextSym is not None:
                if contextSym.getType() in ("f32", "Vec3f", "f64"):
                    contextSym.isLateRodata = True
