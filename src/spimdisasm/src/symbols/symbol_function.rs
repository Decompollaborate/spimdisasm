...
impl SymbolFunction {
    pub(crate) fn new(
        ...
        let relocs = vec![None; instructions.len()];
    ...
    fn process_instr_analysis_result_owned(
        ...
        for (instr_rom, target_vram) in instr_analysis.branch_targets() {
            ...
            */
            ...
            let branch_sym =
                owned_segment.add_symbol(*target_vram, GeneratedBy::Autogenerated, false)?;
    ...
    fn process_instr_analysis_result_referenced(
        ...
        for (instr_rom, target_vram) in instr_analysis.func_calls() {
            ...
            */
            ...
            let func_sym =
                referenced_segment.add_symbol(*target_vram, GeneratedBy::Autogenerated, false)?;
...
impl SymbolFunction {
    fn generate_relocs(&mut self, context: &Context) -> Result<(), SymbolPostProcessError> {
        ...
        for (instr_rom, target_vram) in self.instr_analysis.branch_targets() {
            /*
            if common.GlobalConfig.INPUT_FILE_TYPE == common.InputFileType.ELF:
                if self.getVromOffset(instrOffset) in self.context.globalRelocationOverrides:
                    # Avoid creating wrong symbols on elf files
        ...
        for (instr_rom, target_vram) in self.instr_analysis.branch_targets_outside() {
            /*
            if common.GlobalConfig.INPUT_FILE_TYPE == common.InputFileType.ELF:
                if self.getVromOffset(instrOffset) in self.context.globalRelocationOverrides:
                    # Avoid creating wrong symbols on elf files
        ...
        */
        ...
        for (instr_rom, constant) in self.instr_analysis.constant_per_instr() {
            let instr_index = (*instr_rom - self.ranges.rom().start()).inner() / 4;
            let instr = &self.instructions[instr_index as usize];
...
"
        ...
        */
        ...
        for (instr_rom, (_hi_reg, hi_imm)) in self.instr_analysis.hi_instrs() {
            if !self
                .instr_analysis
