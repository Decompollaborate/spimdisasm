...
pub(crate) enum InstrOpLink {
    ...
    /// Specifically this refers to a symbol from the **global** part of the GOT table.
    Call16RegisterLink {
        vram: Vram,
        rom: Rom,
    ...
    ///
    /// This is the result of calling an address loaded with the `%call16` reloc. See `GpGotGlobal`.
    /// Specifically this refers to a symbol from the **local** part of the GOT table.
    ///
    /// This case is pretty rare, it only has been seen on code generated by the gcc toolchain and
    /// in IDO 7.1 ld.
    Call16LocalRegisterLink { vram: Vram, rom: Rom },
    ...
    fn handle_jalr(&self, instr: &Instruction) -> InstructionOperation {
        ...
        let info = match reg_value {
            GprRegisterValue::RawAddress {
                ...
                GprRegRawAddress::HiLo { hi_rom: _ }
                | GprRegRawAddress::GpRel {}
                | GprRegRawAddress::PairedGpGotLo { upper_rom: _ } => {
                    InstrOpLink::RawRegisterLink {
                        vram: *vram,
                        rom: *setter_rom,
                    }
                }
                GprRegRawAddress::GpGotGlobal { global_entry } => InstrOpLink::Call16RegisterLink {
                    vram: *vram,
                    rom: *setter_rom,
                    global_entry: global_entry.clone(),
                },
                GprRegRawAddress::GpGotLocal {} => InstrOpLink::Call16LocalRegisterLink {
                    vram: *vram,
                    rom: *setter_rom,
                },
