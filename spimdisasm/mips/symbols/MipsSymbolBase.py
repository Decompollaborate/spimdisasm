...
class SymbolBase(common.ElementBase):
    def __init__(self, context: common.Context, vrom: int, inFileOffset: int, vram: int, words: list[int], sectionType: common.FileSectionType):
        super().__init__(context, vrom, inFileOffset, vram, "", words, sectionType)

        self.endOfLineComment: list[str] = []

        contextSym = self.context.addSymbol(self.vram, self.sectionType, isAutogenerated=True)
        contextSym.vromAddress = self.vrom
        ...
        for j in range(0, 4, byteStep):
            ...
            if isByte:
                ...
                if len(self.context.relocSymbols[self.sectionType]) > 0:
                    ...
                    symbolRef = self.context.getSymbol(w, tryPlusOffset=canReferenceSymbolsWithAddends)
