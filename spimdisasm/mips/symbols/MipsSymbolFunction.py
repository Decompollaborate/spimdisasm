...
class SymbolFunction(SymbolText):
    def __init__(self, context: common.Context, vromStart: int, vromEnd: int, inFileOffset: int, vram: int, instrsList: list[instructions.InstructionBase], segmentVromStart: int, overlayType: str|None):
        super().__init__(context, vromStart, vromEnd, inFileOffset, vram, list(), segmentVromStart, overlayType)
    ...
    def _processBranch(self, instr: instructions.InstructionBase, instructionOffset: int, currentVram: int) -> None:
        ...
        labelSym = self.context.addBranchLabel(targetBranchVram, isAutogenerated=True)
    ...
    def _processSymbol(self, luiInstr: instructions.InstructionBase|None, luiOffset: int|None, lowerInstr: instructions.InstructionBase, lowerOffset: int) -> int|None:
        ...
        contextSym = self.context.getSymbol(patchedAddress)
        if contextSym is None:
            if common.GlobalConfig.ADD_NEW_SYMBOLS:
                contextSym = self.context.addSymbol(patchedAddress, isAutogenerated=True)
    ...
    def analyze(self):
        ...
        for instr in self.instructions:
            ...
            if instr.isBranch() or instr.isUnconditionalBranch():
                ...
                funcSym = self.context.addFunction(target, isAutogenerated=True)
        ...
        if instr.isBranch() or instr.isUnconditionalBranch():
            if not common.GlobalConfig.IGNORE_BRANCHES:
                if instr.uniqueId == instructions.InstructionId.J:
                    ...
                    branch = instructionOffset + instr.getBranchOffset()
                    targetBranchVram = self.getVramOffset(branch)
                labelSymbol = self.context.getSymbol(targetBranchVram, tryPlusOffset=False)
            ...
            if not self.pointersRemoved and instructionOffset in self.pointersPerInstruction:
                ...
                if patchedAddress is not None:
                    symbol = self.context.getSymbol(patchedAddress, True, False)
                else:
                    symbol = self.context.getSymbol(address, True)
            ...
            possibleOverride = self.context.getSymbol(instr.getInstrIndexAsVram(), tryPlusOffset=False)
    ...
    def getLabelForOffset(self, instructionOffset: int) -> str:
        if not common.GlobalConfig.IGNORE_BRANCHES and instructionOffset != 0:
            ...
            currentVram = self.getVramOffset(instructionOffset)
            labelSym = self.context.getSymbol(currentVram, tryPlusOffset=False)
