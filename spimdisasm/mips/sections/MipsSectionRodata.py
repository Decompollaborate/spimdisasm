        ...
        for w in self.words:
            ...
            if relocInfo is not None:
                if relocInfo.vram is not None:
                    ...
                    relocVram += w
                    if relocInfo.referencedSection == common.FileSectionType.Text:
                        self.addJumpTableLabel(relocVram, isAutogenerated=True)
                print(f"{vram:X}, {relocInfo}")
                if relocInfo.name is not None and relocInfo.name.startswith("."):
                    sectType = common.FileSectionType.fromStr(relocInfo.name)
                    # relocInfo.sectionType = sectType

                    relocName = f"{relocInfo.name}_{w:06X}"
                    ...
                    if sectType == common.FileSectionType.Text:
                        ...
                        #     relocInfo.jumptableLabel = True
                    relocInfo.name = relocName
                    # print(relocInfo.name, f"{w:X}")
            vram += 4
