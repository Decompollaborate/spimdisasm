...
class SectionText(SectionBase):
    def __init__(self, context: common.Context, vrom: int, vram: int|None, filename: str, array_of_bytes: bytearray):
        super().__init__(context, vrom, vram, filename, array_of_bytes, common.FileSectionType.Text)
    ...
    def analyze(self):
        ...
        currentFunctionSym = self.context.getSymbol(self.getVramOffset(instructionOffset), tryPlusOffset=False)
        ...
        while index < nInstr:
            ...
            if instr.isBranch() or instr.isUnconditionalBranch():
                    ...
                    if not isLikelyHandwritten:
                        ...
                        while j >= 0:
                            if (branch + instructionOffset) < funcsStartsList[j] * 4:
                                ...
                                funcSymbol = self.context.getSymbol(vram, tryPlusOffset=False)
                ...
                self.context.addFunction(target, isAutogenerated=True)
            ...
            funcSymbol = self.context.getSymbol(currentVram + 8, tryPlusOffset=False)
            if funcSymbol is not None and funcSymbol.isTrustableFunction(self.isRsp):
                functionEnded = True
        ...
        for startIndex in range(startsCount):
            ...
            if common.GlobalConfig.DISASSEMBLE_UNKNOWN_INSTRUCTIONS or not hasUnimplementedIntrs:
                funcSymbol = self.context.addFunction(vram, isAutogenerated=True)
                # funcSymbol.isDefined = True
            else:
                contextSym = self.context.getSymbol(vram, False)
                if contextSym is not None:
                    contextSym.isDefined = True
                elif common.GlobalConfig.ADD_NEW_SYMBOLS:
                    contextSym = self.context.addSymbol(vram, isAutogenerated=True)
                    contextSym.isDefined = True

            func = symbols.SymbolFunction(self.context, self.getVromOffset(localOffset), self.inFileOffset + localOffset, vram, instrsList[start:end])
            func.index = i
