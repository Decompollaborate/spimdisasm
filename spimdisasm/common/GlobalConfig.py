...
class GlobalConfig:
    ...
    def STRING_GUESSER(self) -> bool:
        return GlobalConfig.RODATA_STRING_GUESSER_LEVEL > 0
    #! @deprecated
    @STRING_GUESSER.setter
    def STRING_GUESSER(self, value: bool) -> None:
        if value:
            if GlobalConfig.RODATA_STRING_GUESSER_LEVEL <= 0:
                GlobalConfig.RODATA_STRING_GUESSER_LEVEL = 1
        else:
            GlobalConfig.RODATA_STRING_GUESSER_LEVEL = 0

    #! @deprecated
    @property
    def AGGRESSIVE_STRING_GUESSER(self) -> bool:
        return GlobalConfig.RODATA_STRING_GUESSER_LEVEL > 1
    #! @deprecated
    @AGGRESSIVE_STRING_GUESSER.setter
    def AGGRESSIVE_STRING_GUESSER(self, value: bool) -> None:
        if value:
            if GlobalConfig.RODATA_STRING_GUESSER_LEVEL <= 2:
                GlobalConfig.RODATA_STRING_GUESSER_LEVEL = 9
        else:
...
Sets the level for the rodata C string guesser. Smaller values mean more
conservative methods to guess a string, while higher values are more agressive.
0 (and negative) completely disables the guessing feature. Defaults to {GlobalConfig.RODATA_STRING_GUESSER_LEVEL}.

A C string must start at a 0x4-aligned region, which is '\\0' terminated and
padded with '\\0's until a 0x4 boundary.

level 0: Completely disable the guessing feature.
level 1: The most conservative guessing level. Imposes the following
         restrictions:
            - Do not try to guess if the user provided a type for the symbol.
            - Do no try to guess if type information for the symbol can be
              inferred by other means.
            - A string symbol must be referenced only once.
            - Strings must not be empty.

level 2: A string no longer needs to be referenced only once to be considered a
         possible strings. This can happen because of a deduplication
         optimization.
...
"""
        ...
        backendConfig.add_argument("--string-guesser", help=f"DEPRECATED, prefer `--rodata-string-guesser`. Toggles the string guesser feature. Defaults to {GlobalConfig.STRING_GUESSER}", action=Utils.BooleanOptionalAction)
        backendConfig.add_argument("--aggressive-string-guesser", help=f"DEPRECATED, prefer `--rodata-string-guesser`. Makes the string guesser feature to be more aggressive when trying to detect strings. Requires `--string-guesser` to be enabled. Defaults to {GlobalConfig.AGGRESSIVE_STRING_GUESSER}", action=Utils.BooleanOptionalAction)
    ...
    def parseArgs(cls, args: argparse.Namespace):
        ...
        if args.custom_suffix:
            GlobalConfig.CUSTOM_SUFFIX = args.custom_suffix
