...
class ContextSymbol:
    ...
    def getCsvHeader() -> str:
        ...
        output += "isGot,isGotGlobal,isGotLocal"
        return output
    ...
    def toCsv(self) -> str:
        ...
        output += f"{self.isGot},{self.isGotGlobal},{self.isGotLocal}"
        return output
...
class ContextRelocInfo():
    ...
    def getDefaultName(self) -> str:
        if self.jumptableLabel:
            return f".L{self.vram:08X}"

        # if GlobalConfig.AUTOGENERATED_NAMES_BASED_ON_SECTION_TYPE:
        #     if self.sectionType == FileSectionType.Rodata:
        #         return f"RO_{self.address:06X}"
        #     if self.sectionType == FileSectionType.Bss:
        #         return f"B_{self.address:06X}"

        return f"D_{self.vram:06X}"

    def getName(self) -> str:
        if self.referencedSection is not None and self.vram is not None:
            return self.getDefaultName()
        return self.name
