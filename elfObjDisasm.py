...
def elfObjDisasmMain():
    ...
    for sectionType, sectionBytes in elfFile.progbits.items():
        ...
        # TODO: use vrom
        if sectionType == spimdisasm.common.FileSectionType.Text:
            processedFiles[sectionType] = (outputFilePath, spimdisasm.mips.sections.SectionText(context, 0, 0, 0, inputPath.stem, sectionBytes))
        if sectionType == spimdisasm.common.FileSectionType.Data:
            processedFiles[sectionType] = (outputFilePath, spimdisasm.mips.sections.SectionData(context, 0, 0, 0, inputPath.stem, sectionBytes))
        if sectionType == spimdisasm.common.FileSectionType.Rodata:
            processedFiles[sectionType] = (outputFilePath, spimdisasm.mips.sections.SectionRodata(context, 0, 0, 0, inputPath.stem, sectionBytes))
    ...
    if elfFile.nobits is not None:
        ...
        processedFiles[spimdisasm.common.FileSectionType.Bss] = (outputFilePath, spimdisasm.mips.sections.SectionBss(context, 0, 0, 0, elfFile.nobits, inputPath.stem))
